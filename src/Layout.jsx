...
import { Home, BookOpen, LogOut, LogIn } from 'lucide-react';
...
export default function Layout({ children, currentPageName }) {
  ...
  const handleLogout = async () => {
    try {
      ...
      // Logout through Base44 (will handle redirect)
          ...
          <div className="flex items-center justify-between">
            ...
            <Link to={createPageUrl('Home')}>
              ...
              >
                ...
                <div>
                  <h1 
                    className="text-lg md:text-2xl font-black tracking-wider retro-text"
                    style={{
                  ...
                  <p className="text-[10px] md:text-xs font-bold text-cyan-400" style={{ fontFamily: 'monospace', letterSpacing: '1px' }}>
                    WALLET
                  </p>
                </div>
              </motion.div>
            ...
            <div className="flex items-center gap-1 md:gap-2">
              <Link to={createPageUrl('Home')}>
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className={`flex items-center gap-1 md:gap-2 px-2 md:px-4 py-1.5 md:py-2 rounded-none font-bold text-xs md:text-sm transition-all border-2 md:border-4 ${
                ...
                <>
                  {isAuthenticated ? (
                    <motion.button
                      ...
                      className="flex items-center gap-1 md:gap-2 px-2 md:px-4 py-1.5 md:py-2 rounded-none font-bold text-xs md:text-sm bg-red-600 text-white border-2 md:border-4 border-red-800 hover:bg-red-500 transition-all"
                      style={{ fontFamily: 'monospace' }}
                    >
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      onClick={handleLogin}
                      className="flex items-center gap-1 md:gap-2 px-2 md:px-4 py-1.5 md:py-2 rounded-none font-bold text-xs md:text-sm bg-green-500 text-white border-2 md:border-4 border-green-700 hover:bg-green-400 transition-all"
                      style={{ fontFamily: 'monospace' }}
                    >
